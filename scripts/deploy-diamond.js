...
async function deployDiamond (log) {
  ...
  if(log) console.log('DiamondCutFacet deployed:', diamondCutFacet.address)

  // deploy Diamond
  const Diamond = await ethers.getContractFactory('ArbRoot')
  ...
  // deploy facets
  if(log) console.log('')
  if(log) console.log('Deploying facets')
  const FacetNames = [
    // 'DiamondLoupeFacet',
    // 'OwnershipFacet',
    // 'PremiumPlanFacet',
  ...
  for (const FacetName of FacetNames) {
    ...
    if(log) console.log(`${FacetName} deployed: ${facet.address}`)
    cut.push({
